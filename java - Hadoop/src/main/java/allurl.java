...
public class allurl extends Configured implements Tool {
    public static Path urlspath = new Path("/user/e.pitanov/data/url.data");
    public static Path qidspath = new Path("/user/e.pitanov/data/queries.tsv");
    //public static Path urlspath = new Path("./url.data/url.data");
    //public static Path qidspath = new Path("./queries.tsv");

    public static class allurlMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        protected void map(LongWritable key, Text value, Context context)
            ...
            for (int i = 0; i < links.length; i++){
                if (url_id.containsKey(links[i])) {
                    ...
                    context.write(new Text(url_id.get(links[i])), new Text(sb.toString()));
                }
            }
        }
    ...
    public static class allurlReducer extends Reducer<Text, Text, Text, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values, Context context)
            ...
            for (String q: stats.keySet()){
                ...
                multipleOutputs.write(new Text(q + "\t" + key.toString()), new Text(sb.toString()), "./UQDF");
            }
            ...
            multipleOutputs.write(key, new Text(sb.toString()), "./UDF");
        }
    }
    ...
    private Job getJobConf(final String input, final String output) throws IOException {
        ...
        job.setJarByClass(allurl.class);
        job.setInputFormatClass(TextInputFormat.class);
        job.setJobName(allurl.class.getCanonicalName());
        job.setInputFormatClass(TextInputFormat.class);
        ...
        job.setMapperClass(allurlMapper.class);
        job.setReducerClass(allurlReducer.class);
    ...
    static public void main(final String[] args) throws Exception {
        final int ret = ToolRunner.run(new allurl(), args);
        System.exit(ret);
    }
}
